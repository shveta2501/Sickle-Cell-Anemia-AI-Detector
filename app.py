...
GENETIC_PANEL = {
    "Sickle Cell Anemia": {
        "gene": "HBB",
        "wild_type": "CCTGAGGAG",  # Normal Codon 6 context
        "mutant": "CCTGTGGAG",     # Sickle Codon 6 context
        "carrier_code": "W",       # IUPAC code for A or T (Weak)
        "locus": "Chromosome 11, HBB Locus (Codon 6)"
    },
    "Beta Thalassemia (Nonsense)": {
        "gene": "HBB",
        "wild_type": "CAGGAGGCT",  # Normal Codon 39 context
        "mutant": "TAGGAGGCT",     # Nonsense stop codon mutation
        "carrier_code": "Y",       # IUPAC code for C or T (pYrimidine)
        "locus": "Chromosome 11, HBB Locus (Codon 39)"
    }
}

# Target Disease Selection Feature Box
...
if uploaded_file is not None:
    ...
    for record in SeqIO.parse(data_stream, "fasta"):
        ...
        # 5. Advanced Molecular Biology Validation (Central Dogma Tracking)
        st.subheader("🧬 Central Dogma Pipeline Validation")
        bio_seq = Seq(dna_sequence)
        mrna_seq = bio_seq.transcribe()
        
        try:
            protein_seq = mrna_seq.translate(to_stop=False) 
        ...
        # Algorithmic Evaluation of Zygosity (Handles multi-allele stacks or clinical IUPAC codes)
        ...
        # Condition A: Heterozygous / Carrier Check
        if (has_wild_type and has_mutant) or has_iupac_carrier:
            ...
            if has_iupac_carrier:
                ...
                # Display the IUPAC variant center context
                html_highlighted_context = f"...{dna_sequence[max(0, idx_c-10):idx_c]}<span style='background-color:#FEEBC8; color:#744210; padding:2px; font-weight:bold;'>{dna_sequence[idx_c-4:idx_c+5]} (IUPAC '{carrier_char}')</span>{dna_sequence[idx_c+5:idx_c+15]}..."
            else:
            ...
            st.write("Heterozygous profile detected. One normal copy and one mutated copy found. The patient is a carrier.")
            
        # Condition B: Homozygous Normal
            ...
            st.write("Both alleles are normal. No mutation detected within the targeted diagnostic frame.")
            
        # Condition C: Homozygous Mutant
            ...
            st.write("Homozygous mutation detected. No normal wild-type sequences found within the targeted validation window.")
        else:
            st.error("⚠️ REFERENCE TARGET INCONCLUSIVE")
            st.write("The system cannot map the targeted reference segments. Please ensure the sequence covers the HBB target coordinates.")

        # Display Interactive Mutation Highlights to the Clinician
        if html_highlighted_context:
            ...
            if selected_disease == "Sickle Cell Anemia":
                st.info(f"**Target Parameters Logged:** Wild-Type Codon: `GAG` | Mutated Codon: `GTG` | Associated Registry ID: `rs334`")
        ...
        if selected_disease == "Sickle Cell Anemia":
            ...
            st.write("Modeling **MstII Restriction Enzyme** Cleavage Metrics (Recognition Site: `CCTNAGG`)")
            
            mstII_pattern = re.compile(r"CCT[ATGC]AGG")
            cut_sites = [m.start() for m in mstII_pattern.finditer(dna_sequence)]
            
            # FIXED: Substring check targeting exact initialization prefixes
            if "AA" in genotype_status:
                ...
                if cut_sites:
                    st.write(f"Enzyme fragments cleavage coordinates resolved at index values: `{cut_sites}`")
                ...
                st.write("Structural modification changes the site alignment pattern, causing fragment size expansion.")
                ...
                st.write("Both uncut mutant fragments (large) and cleaved normal fragments (small) will appear under gel electrophoresis assays.")
            
            st.markdown("---")

        # 8. Interactive Laboratory Report Preview Engine Module
        st.subheader("📋 Laboratory Clinical Report Data Summary")
        
        report_data = {
            "Patient Identifier Key": patient_id,
            "Targeted Disease Panel": selected_disease,
            "Resolved Clinical Genotype": genotype_status,
            "Sequence Length Profile": f"{len(dna_sequence)} Base Pairs",
            "Regulatory Compliance Flag": "For Investigation Use Only (RUO)"
        }
        st.json(report_data)
import streamlit as st
from Bio import SeqIO
...
if uploaded_file is not None:
    ...
    # Parse records safely into a list to prevent UI rendering explosion inside loops
    records = list(SeqIO.parse(data_stream, "fasta"))
    
    if len(records) == 0:
        st.error("No valid FASTA records detected in file.")
    else:
        # If multiple records exist, let user select which patient to inspect
        if len(records) > 1:
        ...
        try:
            # Clean translation by rounding to nearest codon multiple to prevent bio warnings
